...
public interface FixUpTaskRepository extends JpaRepository<FixUpTask, Integer> {
	...
	@Query("select f from FixUpTask f where (f.ticker like '%?1%' or f.description like '%?1%' or f.address like '%?1%') and f.fixUpTaskCategory.id = ?2 and f.warranty.id = ?3 and f.maximumPrice > ?4 and f.maximumPrice < ?5 and f.timeLimit > ?6 and f.timeLimit < ?7")
	...
	@Query("select f.customer from FixUpTask f where f.description like '%?1%' or f.address like '%?1%'")
